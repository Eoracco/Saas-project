...
export async function parseFeedUrl(url: string) {
    try {
        ...
        throw new Error(
            `Failed to fetch or parse RSS feed: ${error instanceof Error ? error.message : "Unknown error"}`
        );
    }
}

// Extract feed-level metadata from parsed RSS feed
...
export function extractArticles(
    ...
    return feed.items.map((item) => {
        ...
        const rawCategories = (item.categories ||
            itemAny.category || []) as RssCategory[];
