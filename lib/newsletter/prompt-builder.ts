...
// Builds articles summaries for AI prompt
...
// @returns Formatted string with all articles summaries
...
// Extracts non-empty settings and formats them for AI prompt.
// Settings guide the AI on tone, style, and required content.
...
function buildSettingsContext(
    ...
    const addSetting = (label: string, value: string | undefined | null) => {
        if (value) {
            settingLines.push(`${label}: ${value}`); // Added space for better formatting
        }
    };
    ...
    if (settings.disclainmerText) {
        settingLines.push(`Required disclaimer text to include at the end: "${settings.disclainmerText}"`);
    }
...
function buildBodyRequirements(params: NewsletterPromptParams): string[] {
    ...
    if (params.settings?.disclainmerText) {
        requirements.push(
...
function buildImportantNotes(params: NewsletterPromptParams): string {
    ...
    const hasDisclaimer = params.settings?.disclainmerText;
...
export function buildNewsletterPrompt(params: NewsletterPromptParams): string {
    ...
    return promptSections.filter(section => section !== "").join("\n");
}